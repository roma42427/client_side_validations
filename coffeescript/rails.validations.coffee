...
window.ClientSideValidations.enablers =
  ...
  input: (input) ->
    ...
    $input.filter('[id$=_confirmation]').each ->
      ...
      # FocusedIn is needed so we don't get error message before we even reach the field
      confirmationElement.on 'focusin.ClientSideValidations', ->
        confirmationElement.data('focusedIn', true)
...
window.ClientSideValidations.validators =
  ...
  local:
    ...
    confirmation: (element, options) ->
      ...
      if element.data('focusedIn') && !regex.test(parentElement.val())
        return options.message
